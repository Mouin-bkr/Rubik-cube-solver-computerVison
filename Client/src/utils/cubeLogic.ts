import type { CubeState, Face, Move } from "../types/cube";
...
export function getMoveNotation(move: Move): string {
  ...
  if (direction === "counterclockwise") return `${face}'`;
  return face;
}
...
export function parseMoveNotation(notation: string): Move | null {
  ...
  if (modifier === "2") {
    return { face, direction: "clockwise", double: true };
  } else if (modifier === "'") {
    return { face, direction: "counterclockwise" };
  } else {
    return { face, direction: "clockwise" };
  }
}
